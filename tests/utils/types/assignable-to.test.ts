...
describe("AssignableTo", () => {
	...
	describe("subtype relations", () => {
		it("should resolve to true when a literal flows into its widening primitive", () => {
			expectTypeOf<AssignableTo<"a", string>>().toEqualTypeOf<true>();
		});

		it("should resolve to true when a narrow numeric literal flows into `number`", () => {
			expectTypeOf<AssignableTo<1, number>>().toEqualTypeOf<true>();
		});

		it("should resolve to true when `true` flows into `boolean`", () => {
			expectTypeOf<AssignableTo<true, boolean>>().toEqualTypeOf<true>();
		});

		it("should resolve to false when a primitive flows into one of its literals", () => {
			expectTypeOf<AssignableTo<string, "a">>().toEqualTypeOf<false>();
		});

		it("should resolve to false when `boolean` flows into the literal `true`", () => {
	...
	describe("union types (non-distributive)", () => {
		...
		it("should resolve to true when a single member flows into its union", () => {
			expectTypeOf<AssignableTo<"a", "a" | "b">>().toEqualTypeOf<true>();
		});

		it("should resolve to true when a primitive flows into a union that contains it", () => {
			expectTypeOf<
				AssignableTo<number, number | string>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to true when the supertype union has an extra member", () => {
			expectTypeOf<
				AssignableTo<"a" | "b", "a" | "b" | "c">
			>().toEqualTypeOf<true>();
		});

		it("should resolve to false when the candidate union has an extra member", () => {
			expectTypeOf<
	...
	describe("object types", () => {
		...
		it("should resolve to true when the candidate has extra keys (width subtyping)", () => {
			expectTypeOf<
				AssignableTo<{ a: string; b: number }, { a: string }>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to true when an optional key is satisfied by the candidate", () => {
			expectTypeOf<
				AssignableTo<{ a: string }, { a?: string }>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to true when any object shape flows into the empty object", () => {
			expectTypeOf<
				AssignableTo<{ a: string }, NonNullable<unknown>>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to false when the candidate is missing a required key", () => {
			expectTypeOf<
		...
		it("should resolve to false when an optional candidate flows into a required key", () => {
			expectTypeOf<
				AssignableTo<{ a?: string }, { a: string }>
			>().toEqualTypeOf<false>();
		});

		it("should resolve to false when the empty object flows into a shape with required keys", () => {
			expectTypeOf<
	...
	describe("array types", () => {
		...
		it("should resolve to true when a narrow element array flows into a widened element union (covariance)", () => {
			expectTypeOf<
				AssignableTo<string[], (string | number)[]>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to true when a mutable array flows into its `readonly` form", () => {
			expectTypeOf<
				AssignableTo<string[], readonly string[]>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to true when a tuple flows into an array of its element type", () => {
			expectTypeOf<
				AssignableTo<[string], string[]>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to false when a widened element union flows into a narrower element array", () => {
			expectTypeOf<
				AssignableTo<(string | number)[], string[]>
			>().toEqualTypeOf<false>();
		});

		it("should resolve to false when a `readonly` array flows into its mutable form", () => {
			expectTypeOf<
				AssignableTo<readonly string[], string[]>
			>().toEqualTypeOf<false>();
		});

		it("should resolve to false when an array flows into a same-element tuple", () => {
			expectTypeOf<
				AssignableTo<string[], [string]>
			>().toEqualTypeOf<false>();
		});
	...
	describe("tuple types", () => {
		...
		it("should resolve to true when a mutable tuple flows into a readonly tuple", () => {
			expectTypeOf<
				AssignableTo<[string, number], readonly [string, number]>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to false when a readonly tuple flows into a mutable tuple", () => {
			expectTypeOf<
				AssignableTo<readonly [string, number], [string, number]>
			>().toEqualTypeOf<false>();
		});

		it("should resolve to true when a fixed tuple flows into one with an optional trailing element", () => {
			expectTypeOf<
				AssignableTo<[string, number], [string, number?]>
			>().toEqualTypeOf<true>();
		});

		it("should resolve to true when a fixed tuple flows into one with rest elements of the matching type", () => {
			expectTypeOf<
	...
	describe("function types", () => {
		...
		it("should resolve to true when a function returns a subtype of the expected return type", () => {
			type A = () => string;
			type B = () => "a";
		...
		describe("with parameter contravariance", () => {
			...
			it("should resolve to true when a function accepts a wider parameter than expected", () => {
				expectTypeOf<AssignableTo<A, B>>().toEqualTypeOf<true>();
			});

			it("should resolve to false when a function accepts a narrower parameter than expected", () => {
		...
		describe("with parameter count variance", () => {
			...
			it("should resolve to true when a function takes fewer parameters than expected", () => {
				expectTypeOf<AssignableTo<A, B>>().toEqualTypeOf<true>();
			});

			it("should resolve to false when a function takes more parameters than expected", () => {
				expectTypeOf<AssignableTo<B, A>>().toEqualTypeOf<false>();
			});
		});

		it("should resolve to false when parameter types are unrelated", () => {
	...
	describe("top, bottom and exotic types", () => {
		...
		it("should resolve to true for any type flowing into `unknown`", () => {
			expectTypeOf<AssignableTo<string, unknown>>().toEqualTypeOf<true>();
		});

		it("should resolve to true for `any` flowing into any type", () => {
		...
		it("should resolve to true for `unknown` flowing into `any`", () => {
			expectTypeOf<AssignableTo<unknown, any>>().toEqualTypeOf<true>();
		});

		it("should resolve to true for `never` flowing into any type", () => {
			expectTypeOf<AssignableTo<never, string>>().toEqualTypeOf<true>();
		});

		it("should resolve to true for `never` flowing into `any`", () => {
			expectTypeOf<AssignableTo<never, any>>().toEqualTypeOf<true>();
		});

		it("should resolve to true for `never` flowing into `unknown`", () => {
			expectTypeOf<AssignableTo<never, unknown>>().toEqualTypeOf<true>();
		});

		it("should resolve to false when `unknown` flows into a concrete type", () => {
			expectTypeOf<
				AssignableTo<unknown, string>
			>().toEqualTypeOf<false>();
		});

		it("should resolve to false when a concrete type flows into `never`", () => {
		...
		it("should resolve to true when `undefined` flows into `void`", () => {
	...
	describe("unrelated shapes", () => {
		...
		it("should resolve to false when comparing primitive to object", () => {
			expectTypeOf<
				AssignableTo<string, { a: string }>
			>().toEqualTypeOf<false>();
		});

		it("should resolve to false when `null` flows into `undefined`", () => {
			expectTypeOf<
				AssignableTo<null, undefined>
			>().toEqualTypeOf<false>();
		});

		it("should resolve to false when `undefined` flows into `null`", () => {
			expectTypeOf<
