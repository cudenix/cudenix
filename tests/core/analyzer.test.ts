...
describe("usesContext", () => {
	...
	describe("memoization", () => {
		it("should honor toString overrides before analysis and preserve cached verdicts", () => {
			const cached = () => 1;
			const fresh = () => 1;
...
describe("usesResponseMetadata", () => {
	describe("metadata-independent access", () => {
		...
		it("should return false for direct non-response field access", () => {
	...
	describe("memoization", () => {
		it("should honor toString overrides before analysis and preserve cached verdicts", () => {
			...
			expect(usesResponseMetadata(cached)).toBe(false);
			expect(usesResponseMetadata(responseHandler)).toBe(false);
		});
	});
});
...
describe("analyzeHandler", () => {
	it("should distinguish direct response fields", () => {
		expect(analyze((context) => context.response.headers)).toEqual({
			...
			needsMemory: false,
			needsRequest: false,
			needsResponseContent: false,
			needsResponseCookies: false,
			needsResponseHeaders: true,
		...
		expect(
			...
			needsMemory: false,
			needsRequest: false,
			needsResponseContent: true,
			needsResponseCookies: true,
			needsResponseHeaders: false,
			needsResponseMetadata: true,
			needsServer: false,
			needsStore: false,
		});
